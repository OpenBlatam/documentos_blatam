#!/usr/bin/env python3
"""
Organizador Permanente para Proyecto Frontier
Sistema de organización inteligente y permanente con IA avanzada
"""
...
class PermanentOrganizer:
    def __init__(self, root_path: str = "/Users/adan/frontier"):
        self.root_path = Path(root_path)
        self.config = self._load_permanent_config()
        self.organization_log = self.root_path / "logs" / "permanent_organization.log"
        self.analysis_cache = {}
        
        # Crear directorio de logs
        self.organization_log.parent.mkdir(exist_ok=True)
    
    def _load_permanent_config(self) -> Dict:
        """Carga configuración permanente del sistema"""
        return {
            "version": "4.0",
            ...
            "categories": {
                ...
                "08_AI_Artificial_Intelligence": {
                    ...
                    "keywords": [
                        ...
                        "ethics", "governance", "training", "development"
                    ],
                    "file_patterns": [
                ...
                "13_Legal_Compliance": {
                    ...
                    "keywords": [
                        ...
                        "property", "data", "protection", "regulatory", "audit"
                    ],
                    "file_patterns": [
            ...
            "settings": {
                ...
                "semantic_weight": 0.7,
                "pattern_weight": 0.3,
                "file_types": [".md", ".py", ".js", ".ts", ".html", ".pdf", ".docx", ".txt", ".json", ".yaml", ".yml"],
                "exclude_patterns": ["*.tmp", "*.temp", "*.bak", "*.old", "*.log", "*.cache"],
                "min_file_size": 50,
                "max_analysis_size": 2000000,  # 2MB
                "auto_organize": True,
    ...
    def analyze_file_permanent(self, file_path: Path) -> Dict:
        """Análisis permanente avanzado de archivo"""
        try:
            # Verificar cache
            ...
            if file_path.suffix.lower() in ['.md', '.txt', '.py', '.js', '.ts']:
                try:
                    ...
                    content_analysis = self._analyze_content_permanent(content)
                except:
                    pass
            
            # Análisis de nombre de archivo
            filename_analysis = self._analyze_filename_permanent(filename)
            
            # Combinar análisis
            combined_analysis = self._combine_analysis_permanent(
    ...
    def _analyze_content_permanent(self, content: str) -> Dict:
        """Análisis permanente del contenido"""
        keyword_scores = {}
        total_keywords = 0
    ...
    def _analyze_filename_permanent(self, filename: str) -> Dict:
        """Análisis permanente del nombre de archivo"""
        pattern_scores = {}
        
        for category_id, category_info in self.config["categories"].items():
            score = 0
            for pattern in category_info["file_patterns"]:
                if self._match_pattern_permanent(filename, pattern):
                    score += 1
    ...
    def _combine_analysis_permanent(self, content_analysis: Dict, filename_analysis: Dict, filename: str) -> Dict:
        """Combina análisis permanente"""
        ...
        if combined_confidence < self.config["settings"]["confidence_threshold"]:
            ...
            file_type = self._get_file_type_category_permanent(filename)
            return {
                "confidence": 0.5,
    ...
    def _get_file_type_category_permanent(self, filename: str) -> str:
    ...
    def _match_pattern_permanent(self, filename: str, pattern: str) -> bool:
        """Verifica si archivo coincide con patrón"""
        pattern = pattern.replace("*", ".*")
        return bool(re.search(pattern, filename, re.IGNORECASE))
    
    def organize_permanent(self) -> Dict:
        """Organización permanente"""
        self.log("Iniciando organización permanente...")
        ...
        for file_path in files_to_organize:
            try:
                # Análisis permanente
                analysis = self.analyze_file_permanent(file_path)
                results["processed"] += 1
        ...
        # Análisis detallado
        results["analysis_details"] = self._analyze_results_permanent(results)
        
        self.log(f"Organización permanente completada: {results['organized']} archivos organizados")
        return results
    
    def _analyze_results_permanent(self, results: Dict) -> Dict:
        """Analiza resultados de la organización permanente"""
    ...
    def verify_permanent_organization(self) -> Dict:
        """Verificación permanente de la organización"""
        self.log("Verificando organización permanente...")
        ...
        self.log(f"Verificación permanente completada: {results['score']}/100 puntos")
        return results
    
    def create_permanent_index_files(self):
        """Crea archivos índice permanentes"""
        self.log("Creando archivos índice permanentes...")
        ...
        for category_id, category_info in self.config["categories"].items():
            ...
            if category_path.exists():
                ...
                if not index_file.exists():
                    self._create_permanent_category_index(category_id, category_info, category_path)
        
        self.log("Archivos índice permanentes creados")
    
    def _create_permanent_category_index(self, category_id: str, category_info: Dict, category_path: Path):
        """Crea archivo índice permanente para categoría"""
...
## 🎯 Palabras clave
{', '.join(category_info['keywords'][:10])}...

---
*Categoría: {category_info['name']}*
*Mantenido por: Sistema de Organización Permanente v{self.config['version']}*
"""
        
        with open(category_path / "README.md", 'w', encoding='utf-8') as f:
            f.write(content)
    
    def generate_permanent_report(self, results: Dict) -> str:
        """Genera reporte permanente"""
        report = f"""# 🏗️ REPORTE PERMANENTE DE ORGANIZACIÓN
**Proyecto**: Frontier  
...
---
*Reporte generado por el Organizador Permanente v{self.config['version']}*
"""
...
def main():
    ...
    organizer = PermanentOrganizer(args.root)
    
    if args.mode in ['organize', 'both']:
        print("🏗️ Iniciando organización permanente...")
        results = organizer.organize_permanent()
        organizer.create_permanent_index_files()
        
        # Generar reporte
        report = organizer.generate_permanent_report(results)
        report_file = Path(args.root) / "PERMANENT_ORGANIZATION_REPORT.md"
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(report)
        
        print(f"📄 Reporte permanente guardado en: {report_file}")
    
    if args.mode in ['verify', 'both']:
        print("🔍 Verificando organización permanente...")
        verification = organizer.verify_permanent_organization()
        
        print(f"📊 Puntaje permanente: {verification['score']}/100")
