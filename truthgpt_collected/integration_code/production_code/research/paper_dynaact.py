...
class DynamicActionSpace(nn.Module):
    ...
    def __init__(self, config: DynaActConfig):
        ...
        if config.hidden_dim <= 0:
            raise ValueError(f"hidden_dim debe ser positivo, recibido: {config.hidden_dim}")
        if config.max_action_space_size <= config.min_action_space_size:
            raise ValueError(f"max_action_space_size debe ser > min_action_space_size, recibido: max={config.max_action_space_size}, min={config.min_action_space_size}")
