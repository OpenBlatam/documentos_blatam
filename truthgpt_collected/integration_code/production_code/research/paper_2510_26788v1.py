...
class FP16StabilityModule(nn.Module):
    ...
    def __init__(self, config: Paper2510_26788v1Config):
        ...
        if config.num_heads <= 0:
            raise ValueError(f"num_heads debe ser positivo, recibido: {config.num_heads}")
        if config.hidden_dim % config.num_heads != 0:
            raise ValueError(f"hidden_dim debe ser divisible por num_heads, recibido: hidden_dim={config.hidden_dim}, num_heads={config.num_heads}")
