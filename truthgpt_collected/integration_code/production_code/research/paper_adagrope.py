...
class AdaGroPEConfig(BasePaperConfig):
    """
    Configuración para AdaGroPE (Production-Ready).
    ...
    Attributes:
        ...
        num_groups: Número de grupos para agrupación adaptativa (debe ser > 0)
        group_size: Tamaño de grupo base (debe ser > 0)
        use_adaptive_grouping: Si True, usa agrupación adaptativa
        distance_threshold: Umbral para agrupación (0.0-1.0)
        training_free: Si True, no requiere entrenamiento adicional
    ...
    def validate(self):
        """Valida la configuración de AdaGroPE."""
        super().validate()
        ...
        if self.num_groups <= 0:
            raise ValueError(f"num_groups debe ser > 0, recibido: {self.num_groups}")
        if self.group_size <= 0:
            raise ValueError(f"group_size debe ser > 0, recibido: {self.group_size}")
        if not 0.0 <= self.distance_threshold <= 1.0:
            raise ValueError(f"distance_threshold debe estar en [0, 1], recibido: {self.distance_threshold}")
...
class AdaGroPEModule(BasePaperModule):
    ...
    def __init__(self, config: AdaGroPEConfig):
        ...
        try:
            ...
            logger.error(f"Error inicializando AdaGroPE: {e}")
            raise
    ...
    def get_positional_encoding(self, positions: torch.Tensor) -> torch.Tensor:
        """
        Obtiene encoding posicional adaptativo (Production-Ready).
        ...
        base_positions = base_positions.clamp(0, L_base - 1).long()  # Asegurar base_positions ∈ [0, L_base)
            ...
            logger.error(f"Error en get_positional_encoding: {e}")
            # Retornar embeddings cero en caso de error
            batch_size, seq_len = positions.shape
            return torch.zeros(batch_size, seq_len, self.config.hidden_dim, device=positions.device)
    ...
    def forward(
        ...
        **kwargs
        ...
        """
        Forward pass con AdaGroPE (Production-Ready).
        ...
        try:
            ...
            metadata = {
                ...
                'use_adaptive_grouping': self.config.use_adaptive_grouping,
            ...
            logger.error(f"Error en forward de AdaGroPE: {e}")
