...
class LLMEnsemble(nn.Module):
    ...
    def __init__(self, config: LLMEnsembleConfig):
        ...
        if config.num_models <= 1:
            raise ValueError(f"num_models debe ser > 1, recibido: {config.num_models}")
        if config.ensemble_method not in ["weighted", "majority", "average"]:
            raise ValueError(f"ensemble_method debe ser uno de ['weighted', 'majority', 'average'], recibido: {config.ensemble_method}")
    ...
    def forward(self, model_outputs: List[torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        """
        if len(model_outputs) != self.num_models:
            raise ValueError(f"Se esperaban {self.num_models} salidas de modelos, recibido: {len(model_outputs)}")
