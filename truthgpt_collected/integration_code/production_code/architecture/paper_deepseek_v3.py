...
class MultiHeadLatentAttention(nn.Module):
    ...
    def __init__(self, config: DeepSeekV3Config):
        ...
        if config.hidden_dim % config.num_attention_heads != 0:
            raise ValueError(f"hidden_dim debe ser divisible por num_attention_heads, recibido: hidden_dim={config.hidden_dim}, num_attention_heads={config.num_attention_heads}")
