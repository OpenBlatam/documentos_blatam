...
class AdaptiveSparseAttention(nn.Module):
    ...
    def _compute_attention(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, 
        ...
        if attention_mask is not None:
            # Mask keys (columns) - expand to [batch, 1, 1, seq_len]
        ...
        # Use largest=True for top-k (default, but explicit for clarity)
        topk_values, topk_indices = torch.topk(scores, k, dim=-1, largest=True, sorted=False)
    ...
    def forward(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if x.dim() != 3:
            raise ValueError(
                f"Expected 3D input [batch, seq_len, hidden_dim], got {x.dim()}D tensor with shape {x.shape}"
            )
        if x.size(-1) != self.hidden_dim:
            raise ValueError(
                f"Hidden dimension mismatch: expected {self.hidden_dim}, got {x.size(-1)}. "
        ...
        # Project Q, K, V with contiguous memory layout
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        Returns:
            ...
            - sparsity: Actual sparsity ratio achieved
            ...
            - sparsity_efficiency: How close actual sparsity is to target
        """
        return {
    ...
    def adjust_sparsity(self, new_sparsity: float):
        ...
        if not (0.0 <= new_sparsity <= 1.0):
            raise ValueError(f"sparsity must be in [0, 1], got {new_sparsity}")
...
class Paper2506_10987v1Module(nn.Module):
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        Returns:
            Dictionary with metrics from the attention module if available
        """
        metrics = {}
        if self.attention is not None:
    ...
    def adjust_sparsity(self, new_sparsity: float):
        ...
        Args:
            new_sparsity: New sparsity ratio (must be in [0, 1])
        """
        if self.attention is not None:
