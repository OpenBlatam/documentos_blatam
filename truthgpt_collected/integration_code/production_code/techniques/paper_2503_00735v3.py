...
class EfficientFlashAttention(nn.Module):
    ...
    def chunked_attention(
        ...
        Divide la secuencia en chunks para reducir la complejidad de O(n²) a O(n²/C).
        ...
        if num_chunks == 0:
            logger.warning(f"Sequence length {seq_len} is 0, returning zero output")
            return output
    ...
    def get_metrics(self) -> Dict[str, float]:
        ...
        Returns:
            Dictionary with efficiency metrics including chunk utilization
        """
        return {
    ...
    def forward(self, x: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        Forward pass con validación mejorada.
