...
class ChainOfDraftModule(nn.Module):
    ...
    def encode_draft_step(self, hidden_state: torch.Tensor) -> torch.Tensor:
        ...
        if hidden_state.dim() == 3:
            ...
            if hidden_state.size(1) == 1:
                ...
                raise ValueError(
                    f"Expected 3D input with size 1 in dim 1, got shape {original_shape}"
                )
        elif hidden_state.dim() != 2:
            raise ValueError(
                f"Expected 2D or 3D input, got {hidden_state.dim()}D with shape {original_shape}"
            )
        
        if hidden_state.size(-1) != self.hidden_dim:
            raise ValueError(
                f"Hidden dimension mismatch: expected {self.hidden_dim}, got {hidden_state.size(-1)}"
            )
    ...
    def forward(self, hidden_states: torch.Tensor, 
                ...
                **kwargs: Any) -> Tuple[torch.Tensor, Dict[str, Any]]:
        """
        ...
        Args:
            ...
            num_draft_steps: Número de pasos de draft (opcional)
        ...
        if num_draft_steps <= 0:
            raise ValueError(
                f"num_draft_steps must be > 0, got {num_draft_steps}"
            )
        
        if num_draft_steps > seq_len:
            logger.warning(
                f"num_draft_steps ({num_draft_steps}) > seq_len ({seq_len}), "
                f"clamping to {seq_len}"
            )
            num_draft_steps = seq_len
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - Token efficiency compared to CoT
            - Latency efficiency
            - Quality retention
            - Experimental setup details
            - Complete table 1 values from paper
        """
        return {
...
class ChainOfDraftModuleWrapper(nn.Module):
    ...
    def get_metrics(self) -> Dict[str, Any]:
        """
        Get metrics from the wrapped Chain of Draft module.
        
        Returns:
            Dictionary with Chain of Draft metrics
        """
