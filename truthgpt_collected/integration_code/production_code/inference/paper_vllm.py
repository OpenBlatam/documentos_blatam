...
class VLLMConfig(BasePaperConfig):
    ...
    target_throughput: float = 4656.0
    
    def validate(self):
        """Valida la configuración."""
        super().validate()
        if self.page_size <= 0:
            raise ValueError(f"page_size debe ser > 0, recibido: {self.page_size}")
        if self.max_batch_size <= 0:
            raise ValueError(f"max_batch_size debe ser > 0, recibido: {self.max_batch_size}")
        if self.target_throughput <= 0:
            raise ValueError(f"target_throughput debe ser > 0, recibido: {self.target_throughput}")
